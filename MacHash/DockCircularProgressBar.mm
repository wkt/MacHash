...
@property NSString *indicateString;
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  // Edge
  {
    ...
    if(self.indicateString == Nil|| [self.indicateString length] <1 )return;
    // Show indicate String.
  ...
  while (1) {
    ...
    countAttrString = 
        [[NSAttributedString alloc] initWithString:indicateString
...
- (void)setIndicateString:(NSString*)aString
{
    NSDockTile* dockTile = [[NSApplication sharedApplication] dockTile];
    DockTileView* dockTileView = (DockTileView*)([dockTile contentView]);
  
    [dockTileView setIndicateString:aString];
}

- (void)setIndicateNunber:(NSUInteger)number
{
    if(number>0){
        [self setIndicateString:[NSString stringWithFormat:@"%ld",number]];

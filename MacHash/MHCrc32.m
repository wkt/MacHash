...
@implementation MHCrc32

- (id)initWith:(unsigned long)inCrc32
{
    self = [super init];
    _inCrc32 = inCrc32;
    _crc32 = _inCrc32 ^ 0xFFFFFFFF;
    return self;
}
...
- (void)update:(const void *)buf length:(size_t)bufLen
{
    ...
    for (size_t i=0; i < bufLen; i++) {
        _crc32 = (_crc32 >> 8) ^ crcTable[ (_crc32 ^ byteBuf[i]) & 0xFF ];
    }
}

- (unsigned long)finish
{
    unsigned long ret = ( _crc32 ^ 0xFFFFFFFF );
    _crc32 = _inCrc32 ^ 0xFFFFFFFF;
    return ret;
}
...
+(id)newInstance
{
    return [[MHCrc32 alloc] initWith:0];
}
@end
